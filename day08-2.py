...
def part2(input_file):
	...
	def execute_instruction_list(register_values, instruction_list, current_max_value):
		for instruction_line in instruction_list:
			...
			if condition_met:
				if instruction_line[1] == "inc":
					...
					register_values[instruction_line[0]] -= instruction_line[2]
			if max(list(register_values.values())) > current_max_value:
					current_max_value = max(list(register_values.values()))
	...
	max_value = execute_instruction_list(register_values, instruction_list, 0)
	return max_value
